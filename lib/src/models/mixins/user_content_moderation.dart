...
abstract class UserContentModerationMixin {
  ...
  /// placed at the top of the comment thread.
  Future distinguish({DistinctionType how, bool sticky: false}) async {
    final data = {'how': _distinctionTypeToString(how), 'id': content.fullname};
    if (sticky && content.isRoot) {
      data['sticky'] = 'true';
    }
  ...
  Future unignoreReports() async => content.reddit
      .post(apiPath['unignore_reports'], {'id': content.fullname});
}
